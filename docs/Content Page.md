# TABLE OF CONTENTS

### 1. [Introduction](1-Introduction.md)
1.1 What you will be learning  
1.2 Preparing for the project  
1.3 Importing Assets  

### 2. [Creating the game](2-Creating the Game.md)
2.1 Creating the game board  
2.2 Scripting  
2.3 Camera  
2.4 Board Data  
2.5 Player Character  

### 3. [Character Development](3-Character Development.md)
3.1 Adding Player Character  
3.2 Moving Player Character  
3.3 Refactoring  

### 4. [Turn System](4-Turn System.md)
4.1 Turn Manager  
4.2 Ticking Turn  

### 5. [Adding a Health System](5-Health System.md)
5.1 Add a Food resource  
5.2 UI Designing  
5.3 Updating the Label using Code  
5.4 Objects in cell and food refill  
5.5 Collecting Health  
5.6 Increasing health count  

### 6. [Adding Obstacles](6-Adding Obstacles.md)
6.1 Add obstacles  
6.2 Refactoring  
6.3 Damaging walls  

### 7. [Beating the game](7-Beating the game.md)
7.1 Finishing a Level  
7.2 Generating a new level  
7.3 Game Over  

### 8. [Animation](8-Animation.md)
8.1 Idle Animation  
8.2 Walking Animation  

### 9. [Adding an Enemy](9-Adding an enemy.md)
9.1 Creating an Enemy  

### 10. [Conclusion](10-Conclusion.md)
